...
public:
    ...
    void adjust_timer(util_timer* timer)
    {
        ...
        // 如果目标定时器不是链表的头结点，则将该定时器从链表中取出，并重新插入链表
        else
        {
    ...
    private:
        void add_timer(util_timer* timer, util_timer* lst_head)
        {
            ...
            // 遍历lst_head节点之后的链表，直到找到一个超时时间大于目标定时器的超时时间的节点，并将目标定时器插入该节点之前
            while(tmp)
            {
            ...
            // 如果遍历完lst_head节点之后的链表，仍未找到超时时间大于目标定时器的超时时间的节点，则将目标定时器插入链表尾部，并把它设置为链表新的尾结点
            if(!tmp)
            {
                prev->next = timer;
                timer->prev = prev;
